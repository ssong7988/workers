...
| 공개 경로 | 실제 처리자 | 용도 | 인증 |
|---|---|---|---|
| `/common/dagster/` | Django, `127.0.0.1:8000` | 최근 실행을 간단히 보여주는 공통 운영 요약 | Django staff 로그인 |
| `/common/dagster/console/` | Dagster, `127.0.0.1:3000` | Dagster가 제공하는 전체 UI, 로그와 수동 실행 | 현재 별도 인증 없음 |
...
|-- /common/dagster/console proxy http://127.0.0.1:3000/common/dagster/console
```

Funnel은 더 구체적인 `/common/dagster/console` 규칙을 먼저 적용한다.

```text
GET https://.../common/dagster/
  -> Funnel의 / 규칙
  -> Django :8000
  -> staff_member_required
  -> 프로젝트 전용 요약 HTML

GET https://.../common/dagster/console/runs
  -> Funnel의 /common/dagster/console 규칙
  -> Dagster :3000/common/dagster/console/runs
  -> Dagster 네이티브 UI
```

두 경로가 비슷해 보여도 `/common/dagster/console/` 요청은 Django를 전혀 지나지
않는다. 그래서 Django의 로그인 장식자도 적용되지 않는다.

## Dagster 자체에도 같은 prefix가 필요한 이유

프록시 규칙만 `/common/dagster/console`로 잡고 Dagster를 `/` 기준으로 실행하면
...
```powershell
$env:DAGSTER_WEBSERVER_PATH_PREFIX = '/common/dagster/console'
```

그러면 Dagster가 모든 내부 URL을 이 prefix 아래에 만든다.

```text
/common/dagster/console/runs
/common/dagster/console/graphql
/common/dagster/console/assets/...
```
...
Django의 `/common/dagster/`는 네이티브 UI를 iframe으로 감싼 화면이 아니다.
`report-site/report/dagster_client.py`가 서버 내부에서 다음 로컬 GraphQL로
최근 run을 조회하고 별도 HTML을 렌더링한다.

```text
http://127.0.0.1:3000/common/dagster/console/graphql
```

흐름은 다음과 같다.

```text
사용자 -> /common/dagster/ -> Django view
                         |
...
```text
/example-token/common/dagster/
/example-token/common/dagster/console/
```
...
`/common/dagster/` 요약은 Django staff 로그인을 요구하지만, 현재
`/common/dagster/console/`은 Funnel이 Dagster에 직접 연결하므로 Django 인증을
...
```powershell
# 1. 로컬 Dagster UI
Invoke-WebRequest http://127.0.0.1:3000/common/dagster/console/runs -UseBasicParsing

# 2. Funnel 규칙
& 'C:\Program Files\Tailscale\tailscale.exe' funnel status

# 3. 공개 UI
Invoke-WebRequest https://desktop-477.tailf8d9d1.ts.net/common/dagster/console/runs -UseBasicParsing

# 4. Django 요약은 비로그인 요청이면 admin 로그인으로 302가 정상
curl.exe -I https://desktop-477.tailf8d9d1.ts.net/common/dagster/
```
