...
| 네이버 로그인 | 관심부동산 접근 | `.env`에 자격 증명이 있으면 직접 로그인, 없으면 대기하거나 실패 |
...
06:00 `check-login`은 볼 사람이 없다. 그래서 로그아웃을 발견하면
경고만 하지 않고 직접 로그인한다. 자격 증명은 `credentials.py`가 루트 `.env`의
`NAVER_ID`·`NAVER_PASSWORD`에서 읽는다 — 코드·문서·로그 어디에도 값을 두지
않는다. 둘 중 하나라도 비어 있으면 자동 로그인을 시도하지 않고 예전대로
실패한다.

입력은 사람이 하듯 한 글자씩 타이핑하고(`press_sequentially`), 네이버가
CAPTCHA·인증번호·비밀번호 불일치를 띄우면 재시도하지 않고 멈춘다. 계정이
잠기는 것이 수집 한 번을 놓치는 것보다 나쁘기 때문이다.
