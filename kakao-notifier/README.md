...
### 원본 이미지와 전체 매물 버튼을 함께 보냅니다

카드와 `원본 이미지 보기` 버튼은 **업로드된 원본 이미지 자체**를 엽니다. 채팅방에 보이는 카드는 축소본이라, 전체 해상도(예: 1080×4992)로 볼 수 있도록 이 동작을 유지합니다.

`--link`를 지정하면 `전체 매물 보기` 버튼이 두 번째로 추가됩니다. 따라서 원본 이미지 확대와 웹 리포트 이동을 한 메시지에서 모두 사용할 수 있습니다.
