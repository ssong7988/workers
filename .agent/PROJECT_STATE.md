...
마지막 갱신: 2026-09-04 (매물에 동(building) 필드 추가, 스캔 진단 로그 추가, 백그라운드 Edge 탭 로그인 오탐 완화)
...
- **`Observation`/`Listing`에 `building`(동) 필드를 추가했다(2026-09-04, 마이그레이션 `0004`).** 관심부동산 카드 파서는 이미 동을 읽고 있었지만 finder의 `Listing` 데이터클래스가 버렸다 — 층만으로는 몇 동인지 알 수 없다는 지적으로 finder → API → DB → admin → 리포트 화면까지 이어지도록 연결했다. 통계 화면은 건물을 가로질러 집계하므로 의도적으로 제외했다.
