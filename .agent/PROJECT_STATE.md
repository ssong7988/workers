...
부모의 High를 물려받는다). **H-able 쪽 권한을 낮추는 방법은 없다** - 매니페스트를 고치면
서명이 깨지고 업데이트가 되돌린다. 건드리지 않는다.

Windows UIPI는 낮은 권한 프로세스가 높은 권한 창에 보내는 **입력과 창 메시지를 오류 없이
전부 버린다.** 그래서 수집기가 Medium이면 클릭도 키 입력도 "아무 일도 일어나지 않는" 것으로만
보인다. `window.ensure_input_allowed()`가 수집 전에 두 무결성 수준을 비교하고 낮으면 멈춘다.

**해법: 작업 스케줄러의 "가장 높은 권한으로 실행" 태스크.** `Start-Process -Verb RunAs`는
실행마다 사람이 UAC를 눌러야 해서 평일 18:30 무인 수집과 어긋난다. 태스크는 **등록할 때
한 번만** 관리자 승인이 필요하고 그 뒤 `Start-ScheduledTask`는 프롬프트가 없다.

| 파일 | 역할 |
|---|---|
| `stock-importer/setup-task.ps1` | 태스크 `KB-StockImporter` 등록. 관리자 권한으로 **한 번만** |
| `stock-importer/run-stock.cmd` | 태스크가 실행하는 진입점. 하위 명령을 `data\next-command.txt`에서 읽는다(태스크는 실행 시 인자를 못 받는다). `script:경로`로 시작하면 그 파이썬 파일을 대신 돌린다 - 일회성 진단용 통로다 |
| `stock-importer/run-stock.ps1` | 평상시 진입점. `-Command hable-import` 식으로 부르고 로그를 회수한다. **UAC 없음** |

Dagster도 같은 태스크를 부르면 된다. PowerShell 스크립트의 메시지가 영어인 것은 이 저장소의
기존 규칙이다 - Windows PowerShell 5.1이 `.ps1`을 ANSI로 읽어 한글이 깨진다.

#### 권한을 맞추면 실제로 되는 것 (2026-09-05~06 확인)

| 단계 | 결과 |
|---|---|
| 최소화된 H-able 창 복원 | 동작(`WM_SYSCOMMAND`/`SC_RESTORE`) |
| 로그인 공지 화면 닫기 | 동작. `1837 진행중인 이벤트`·`0114 공지사항`·`화면 종료 안내`가 매 로그인마다 1285를 덮는다 |
| 광고 팝업 닫기 | 동작. always-on-top이라 z-order로는 못 이긴다. **WebView2(`Chrome_RenderWidgetHostHWND`)를 품은 구조로 알아본다** - 제목을 직접 그려서 창 텍스트로는 안 잡힌다 |
| 포그라운드 전환 | ALT를 살짝 눌러 잠금을 푼다. 실패해도 멈추지 않고 z-order만 올린다(`pin_to_top`) - 클릭이 어디로 갈지 정하는 것은 활성 창이 아니라 z-order다 |
...
#### 오래 헤맨 두 가지 (다시 겪지 않도록)

1. **클릭 가드가 "H-able의 창인가"만 봤다.** 로그인마다 뜨는 이벤트·공지 화면도 H-able의
   창이라 통과해 버렸고, 클릭은 툴바가 아니라 그 화면으로 갔다. 지금은 **"대상 화면(1285)의
   창인가"**까지 보고, 아니면 덮은 것을 치우고 다시 올린 뒤 재시도한다. 그래도 안 되면
   **무엇이 가리는지 이름을 적어** 알린다 - 그 한 줄이 없어서 오래 헛돌았다.
...
| 그리드 셀 | **자식 창이 아니다.** 1285의 자식 78개 중 69개가 owner-drawn `AfxWnd120`이고 `SysListView32`는 없다 |
| 툴바(엑셀·인쇄·설정·?·조회) | `Button`이 아니다. `BM_CLICK` 불가, 좌표 클릭뿐. 위치는 화면 그림과 계산값이 일치함을 확인했다 |
| 비트 수 | H-able은 32비트, 우리 Python은 64비트. **구조체를 건네는 메시지는 쓰지 않는다** |
| `kbopenapi.exe` | 매매 API가 아니라 Matomo 행동로그 프록시(`loghub.kbsec.com`) |
| 캐시 파일 | `data/`·`user/`에 설정 ini뿐. 잔고를 파일로 떨구지 않는다 |
| 로그인 팝업 | 로그인할 때마다 `0114 공지사항`·`1837 진행중인 이벤트`·`화면 종료 안내`가 1285를 덮는다. 무인 실행이라면 이것부터 치워야 한다 |

#### 권한 차단이었던 것 / 아니었던 것 (판정 완료)

"안 된다"고 적었던 것들을 승격 상태에서 다시 봤다.

| 항목 | 판정 |
|---|---|
| 최소화 복원, `WM_CLOSE`, 창 조작 | **권한 차단이었다.** 지금은 동작한다 |
| 우클릭 컨텍스트 메뉴 | **권한 차단이었다.** 지금은 뜬다 |
| Ctrl+C로 그리드 복사 | 권한과 무관하게 **안 된다.** 승격 상태에서도 클립보드가 비어 있다 |
...
#### 운영 사고 하나 (2026-09-05 밤)

PostgreSQL이 **포트 5432는 열어둔 채 새 접속을 처리하지 못하는** 상태에 빠졌다. 서비스
항목은 `Stopped`인데 서버 프로세스는 떠 있는 어긋난 상태였고, `psql`도 Django도 타임아웃
이었다. 정식 절차로 멈추고(`pg_ctl stop -m fast`) 서비스를 다시 시작해 되살렸다. 이것
때문에 한동안 `manage.py test`를 못 돌렸다.
...
`manage.py test portfolio` **57개 통과**(전송 테스트 포함). **실제 발송은 아직 안 했다** -
보낼 수집 결과가 없다. 없는 숫자로 메시지를 지어 보내지 않는다.
