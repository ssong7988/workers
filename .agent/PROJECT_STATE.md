...
#### 다시 확인해야 하는 것 (권한 때문에 실패했을 수 있다)

아래는 전부 "안 된다"고 적었다가 UIPI 차단으로 밝혀진 뒤 **판정을 보류한** 것들이다.
권한을 맞춘 뒤 다시 봐야 한다.

- Ctrl+C로 그리드 복사 (클립보드가 비어 있었다)
- 우클릭 컨텍스트 메뉴 (뜨지 않았다)
- 툴바 엑셀 버튼 클릭 → 저장 대화상자 (뜨지 않았다)
- `SysHeader32`에서 컬럼 이름 읽기 (`item_count` 0 - 교차 프로세스 읽기가 막혔을 수 있다)
