...
바뀌므로 이렇게 나눈다. 에 계좌·종목·수량을 넣고, 종목의 에
처럼 적는다. 가 그 수량에 오늘
시세를 곱해 을 만든다 — H-able에서 온 것과 같은 자리라 비중·수익률
화면이 따로 알 필요가 없다.

시세를 못 받으면 **멈춘다.** 어제 가격으로 오늘 평가액을 적으면 조용히 틀린다. 빠진
심볼도 마찬가지로 오류다. 지금은 업비트 공개 API만 안다(인증 없음).

#### 매일 도는 잡 —  (2026-09-06 추가)

**이게 없어서 성과 화면이 첫날에 머물러 있었다.** 기간별 수익률·현금흐름과 낙폭·
자산분류별 수익률은 하루에 한 행씩 쌓이는 으로 그리는데,
를 도는 스케줄이 아예 없었다. 사람이 손으로 돌린 2026-09-06 하루뿐이라
그래프가 비어 있었다.

평일 18:30에 (H-able 잔고 + 최근 7일 거래내역) → 
(코인 시세) 순으로 돈다. 30분 전 이 H-able이 켜져 있는지 미리 확인해
