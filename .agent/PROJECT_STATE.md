...
## Completed: Dagster 운영 설명 확장 (2026-09-09)

**상태: 구현·검증 완료.** `/common/dagster/`를 단순 잡 이름·실행 이력 화면에서 실제 운영 흐름을 이해할 수 있는 설명 화면으로 확장했다. 설명은 `dagster_project/definitions.py`와 각 CLI 진입점을 기준으로 작성했으며 잡 실행 로직 자체는 변경하지 않았다.

- 상단에 Dagster의 역할과 `Schedule → Collect → Store → Deliver` 흐름을 추가했다. Dagster는 수집기가 아니라 Windows 작업·Django 명령을 정해진 순서로 실행하고 상태·실패를 기록하는 스케줄러임을 명시했다.
- 7개 잡 모두에 목적, 실행 방식, 단계별 처리, 결과, 실제 스케줄을 표시한다: `server_check_job`, `pre_scan_health_job`, `hable_ready_job`, `stock_daily_job`, `scan_job`, `morning_report_job`, `restart_report_site_job`.
- 부동산 스캔은 로그인된 Edge(CDP 9222)로 **네이버 부동산 관심단지**를 순회하고 `/api/scans/`로 보내 PostgreSQL에서 조건을 판정하며, 급매·신규 알림 대상이면 저장 과정에서 카카오톡을 보내는 흐름을 적었다.
- 아침 리포트는 07:00 이후 성공 스캔을 확인해 없을 때만 재수집하고, Django `send_digest`가 활성 매물을 읽어 `kakao-notifier`로 텍스트와 웹 버튼을 보내는 흐름을 적었다.
- 금융자산은 H-able `[1285]` 잔고·`[0112]` 거래·`[0354]` 성과, Yahoo Finance·Upbit 시세로 웹 데이터를 갱신한다. 이 잡은 금융자산 카카오 요약을 자동 발송하지 않는다는 경계도 표시했다.
- H-able 사전 점검·세션 유지, 리포트 서버 헬스체크·재시작 범위, 주요 단계 1회 재시도와 최종 `send_alert` 카카오 경고도 각각 구분했다.
- 반응형 상세 카드와 키보드 포커스를 유지하고 상단에 루트 `전체 서비스` 링크를 추가했다. 실행 이력 표와 기존 staff 인증은 그대로다.
- `report` 테스트 60개, `manage.py check`, `makemigrations --check --dry-run`, `git diff --check`가 모두 통과했다.

## Planned Work: 루트 랜딩 페이지 (2026-09-09)

**상태: 구현·테스트·공개 반영 완료.** `https://desktop-477.tailf8d9d1.ts.net/`에 common·property·stock의 설명과 이동 링크를 제공하는 랜딩 페이지를 추가했다. 아래 계획대로 구현했으며 기존 Tailscale Funnel 주소와 Django 서버를 그대로 사용한다.

### 화면과 이동 경로

루트 `/`에서 특정 서비스로 자동 이동하지 않고 서비스 안내 화면을 HTTP 200으로 렌더링한다. 상단에는 `관심 매물 · 금융자산` 제목과 짧은 안내를 두고, 그 아래 common → property → stock 순서로 카드 3개를 배치한다. 각 카드에 한국어 이름·영문 namespace·설명·클릭 가능한 링크와 endpoint 경로를 표시한다. 데스크톱은 3열, 모바일은 1열로 배치하고 기존 화면의 남색 헤더·밝은 배경·시스템 글꼴을 따른다.

| 카드 | 설명 초안 | 이동 버튼과 endpoint |
|---|---|---|
| common · 공통 운영 | 부동산과 금융자산 수집 작업의 스케줄 및 실행 상태를 확인합니다. | `운영 현황` → `/common/dagster/` (관리자 로그인 필요) |
| property · 부동산 | 관심 단지의 조건 충족 매물·급매와 지역·단지별 호가 추이를 확인합니다. | `매물 리포트` → `/property/report/`, `가격 통계` → `/property/statistics/` |
| stock · 금융자산 | 계좌와 자산분류별 비중, 목표 대비 리밸런싱, 기간별 수익률과 낙폭을 확인합니다. | `비중·리밸런싱` → `/stock/allocation/`, `수익률·MDD` → `/stock/performance/` |

`/common/`, `/property/`, `/stock/` 자체는 현재 화면이 없는 namespace이므로 버튼은 위의 실제 하위 endpoint로 연결한다. 별도의 namespace별 소개 페이지는 이번 계획에 포함하지 않는다. Django admin은 기존 `/admin/`을 유지하며 common 아래로 옮기지 않는다. 비활성 Airflow, 수집기 API, Django 인증을 거치지 않는 Dagster 네이티브 console 직행 링크는 랜딩의 기본 링크 목록에 넣지 않는다.

### 구현 순서

1. **라우팅·뷰:** 기존 `report-site/` Django 앱 안에서 `report/views.py`에 `landing` 뷰, `report_site/urls.py`에 루트 named route(`site-home`)를 추가한다. 데이터 조회나 Dagster 요청 없이 설명과 링크만 렌더링해 안내 화면이 외부 서비스 상태에 의존하지 않도록 한다. 기존 부동산·주식·운영·API 라우팅은 유지한다.
2. **템플릿:** `report/templates/report/landing.html`을 추가한다. 카드별 링크는 실제 `<a>` 요소로 제공하고 키보드 포커스, 충분한 클릭 영역, 긴 endpoint 줄바꿈을 지원한다. 새 프런트엔드 프레임워크·차트·실시간 상태 표시·DB 모델은 필요 없다.
3. **URL·토큰 처리:** 링크를 호스트 문자열로 하드코딩하지 않고 Django named URL의 reverse/템플릿 URL 태그로 생성한다. 현재처럼 `REPORT_PATH_TOKEN`이 비어 있으면 `/`에서 모든 안내 링크를 보여 준다. 토큰이 설정되면 `/<TOKEN>/`에 링크가 있는 랜딩을 제공하고 공개 `/`에는 서비스 설명만 제공해 토큰 경로를 공개하거나 자동 리다이렉트로 노출하지 않는다. 기존 하위 화면의 인증은 유지한다.
4. **검증:** DB 없는 랜딩 테스트로 루트 200, 세 설명·다섯 링크, named URL 연결, 토큰 유무에 따른 경로·토큰 비노출을 확인한다. `manage.py check`와 관련 URL/뷰 회귀 검증을 실행한다. 브라우저에서 375px·1440px 레이아웃, 가로 넘침, 키보드 이동과 버튼 목적지를 확인한다. common은 비로그인 시 기존 admin 로그인 이동을 정상 결과로 본다.
5. **운영 반영·인계:** 실행 중이던 report-site가 옛 코드를 들고 있어 `restart-site.ps1`로 재시작했다. Funnel 설정 변경은 필요 없었다. 공개 루트와 하위 링크를 확인했다.

### 완료 조건과 운영 전제

- 공개 루트에서 세 영역의 역할과 각각의 이동 경로를 바로 이해하고 선택할 수 있다.
- 기존 하위 URL·카카오 링크·관리자 인증이 유지되고, 모바일에서도 endpoint와 버튼이 잘리지 않는다.
- 랜딩 요청 자체는 DB 조회·수집·잡 실행·카카오 발송을 유발하지 않는다.
- 구현 파일은 `report/views.py`의 DB 비의존 `landing` 뷰, `report_site/urls.py`의 루트 route, `report/templates/report/landing.html`이다. 짙은 남색 운영 허브 스타일, common·property·stock별 포인트 색, 데스크톱 3열·모바일 1열 카드 레이아웃을 적용했다.
- `report.tests.test_views` 16개, `manage.py check`, `makemigrations --check --dry-run`이 통과했다. 공개 루트는 HTTP 200이며 다섯 endpoint 링크가 HTML에 모두 들어 있음을 확인했다. 기존 property·stock 네 화면은 HTTP 200, 로그인이 필요한 `/common/dagster/`는 기존대로 302다.
- Windows 업데이트 재부팅 뒤 내려갔던 웹 서버는 이후 다시 기동돼 있었고, 새 코드 반영을 위해 재시작했다. 현재 공개 루트와 Tailscale Funnel은 정상이다. Dagster 자체의 현재 실행 상태는 이 랜딩 구현 범위에서 재확인하지 않았다.
...
- **2026-09-08 21:53 `stock_daily_job` 성공.** 팝업 자동 제거 → [1285] 자동 열기 → 조회 → 잔고 저장까지 사람 손 없이 완주했다. 5계좌 18보유를 저장하고 완전 수집일 `2026-09-08`을 확정했다(`/stock/api/status/` 확인). **거래내역([0112])만 못 읽었다** — 화면번호 칸 자리를 Chrome 창이 덮고 있어 `ensure_point_hits_hable()`이 클릭을 거부했다. 가드가 의도대로 동작한 것이다(그 클릭은 Chrome이 가져갔을 것이다). 잔고는 이미 저장됐으므로 잡은 성공으로 끝났다.
- **알려진 제약: 이 자동화는 H-able이 화면 맨 위에 올 수 있어야 한다.** 화면번호 칸과 툴바는 실제 커서 클릭으로만 눌리고, `pin_to_top()`으로도 전체화면 브라우저 같은 창은 이기지 못한다. 18:30 무인 수집 시각에 H-able을 덮는 창을 띄워 두지 않는 것이 전제다.
- **계좌 비밀번호는 조회 화면보다 먼저 열려 있으면 적용되지 않는다(2026-09-08).** `환경설정`에서 저장하기 **전에** 열어 둔 [1285]는 옛 인증 상태를 들고 있어 계속 비밀번호를 요구한다. 저장한 뒤 화면을 닫았다 다시 열면 통과한다.

- **2026-09-08 `stock_daily_job` 실패의 원인은 H-able 화면 상태였다.** KB의 `최선집행기준 설명서 개정 안내` 팝업이 모달로 떠서 본창을 비활성(`IsWindowEnabled == False`)으로 만들었고 [1285]는 닫혀 있었다. 수집기는 설계대로 클릭하지 않고 멈췄다. 재부팅 뒤 Dagster가 꺼져 있어 18:00 `hable_ready_job`이 돌지 않아 사전 경고도 없었다(keepalive 마지막 접속은 09-07 06:00).
- **그 팝업을 스스로 치우게 했다(`stock_importer/hable/window.py`).** 이 창은 제목 표시줄을 직접 그려 `GetWindowText`가 빈 문자열을 돌려주므로 기존의 제목 단어(`공지·이벤트·안내·알림`)·WebView2 판별에 걸리지 않았다. 그래서 **이름이 아니라 본창 비활성 여부**로 판단하는 `blocking_dialogs()`와 새 예외 `BlockedByDialog`를 두고, `dismiss_blocking_dialogs()`가 `WM_CLOSE`만 보내 닫는다. **`WM_CLOSE`만 보내는 것이 안전 근거다** — 제목줄의 X와 같아서 무엇에도 동의하지 않고, 설령 주문 확인이었다 해도 결과는 취소이지 체결이 아니다. 버튼 좌표는 누르지 않는다. 이름이 없으면 `제목 없는 대화상자 (303x265 @ 809,388)`처럼 크기와 자리로 지목한다.
- **단, 로그인·계좌 비밀번호 안내는 절대 닫지 않는다.** `raise_for_auth_dialog()`가 먼저 걸러 `LoggedOut`/`AccountPasswordRequired`를 올린다. 닫으면 사람이 고쳐야 할 상태가 사라진 채 다음 단계에서 다른 얼굴로 실패한다.
- **닫힌 화면도 스스로 연다.** `open_screen()`이 기본메뉴바의 통합검색 칸(표준 `Edit`)에 화면번호를 넣고 Enter를 친다. `ensure_screen_open()`이 없을 때만 부른다. 실측으로 배운 것: (1) `WM_SETTEXT`·`WM_CHAR`·`PostMessage` Enter는 **글자만 바뀌고 화면은 안 열린다** — 이 칸도 실제 클릭·실제 키 입력만 받는다. (2) `keybd_event`는 창이 포그라운드일 때만 닿으므로 클릭으로 포커스를 옮겨야 한다. (3) 그래서 누르기 전에 `ensure_point_hits_hable()`로 그 자리의 주인을 확인한다 — 확인 없이 눌렀을 때 실제로 VS Code가 클릭을 가져갔다. (4) 남은 글자에 이어 붙어 `1285114`가 된 적이 있어 `SetWindowTextW(box, "")`로 먼저 비운다. (5) 넣은 글자는 화면에 보이는데 `GetWindowText`로는 안 읽히므로, 읽히면 검증하고 안 읽히면 그냥 진행한다.
- **`find_screen()`이 화면번호를 타이핑한 입력창 자체를 화면으로 착각하던 것도 고쳤다.** 제목만 보면 `1285`라고 적힌 `Edit`이 조건에 맞는다. 조회 화면은 클래스가 `Afx:`로 시작하므로 `is_screen()`이 둘을 가른다.
- **진단 순서도 고쳤다.** `find_screen()`이 화면을 못 찾으면 `ScreenNotOpen` 전에 `ensure_not_blocked()`를 부른다. 예전에는 진짜 원인이 모달인데 "[1285]를 열어 두라"고 안내해 열 수 없는 화면을 열게 만들었다.
- **실제로 확인했다(2026-09-08 21:38).** 막혀 있던 KB 팝업이 자동으로 닫혔고, 닫혀 있던 [1285]가 자동으로 열렸다. 그 뒤 `계좌 비밀번호를 입력하세요` 모달이 떴고 수집기는 **닫지 않고** `password_required`로 보고했다. 주식 수집기 테스트 108개 통과.
- **남은 사람 손 하나: 계좌 비밀번호다.** H-able `환경설정 → 보안설정 → 계좌설정`에 저장해야 하며, 이 저장소는 그 값을 갖지 않는다(코드·환경변수·로그·DB 어디에도 두지 않는다는 기존 원칙). 저장해 두면 그 뒤로는 사람 손 없이 돈다.
...
- **Django admin은 뿌리의 `.../admin/`이다(2026-09-08 이동).** 부동산과 금융자산 모델을 모두 들고 있어 어느 한 namespace 아래에 둘 이유가 없었다. 옛 `/property/admin/`·`/stock/admin/`은 리다이렉트로 남겼다(`admin.site.urls`를 두 번 마운트하면 `admin:` namespace가 갈라진다). 토큰을 켜면 `.../<TOKEN>/admin/`으로 이동한다. `Observation`의 `exclusion_code` 필터로 제외 사유를 구분해 조회할 수 있다.
...
#### 거래내역 수집 — [0112] (2026-09-06)

**1년치 거래내역을 계좌별로 읽어 `CashFlow`로 넣는다.** 잔고([1285])만으로는
과거를 말할 수 없다 - 지난날의 시세가 없어 평가액을 복원할 방법이 없기 때문이다.
반면 거래내역은 통째로 받아올 수 있어 **확정된 손익**만큼은 과거를 채운다.

이 화면은 [1285]와 다르다.

| 다른 점 | 대응 |
|---|---|
| 거래 하나가 **두 줄**(윗줄 날짜·금액, 아랫줄 종목·단가), 헤더도 두 줄 | `trades.pair_rows()` |
| 계좌를 **하나씩** 골라야 한다(1285는 `전체` 하나로 끝났다) | 콤보 순번으로 선택 |
| 결과가 길면 `다음`으로 이어 받는다 | `read_all_pages()` |
| 조회기간을 넣어야 한다 | 1년을 한 번에 받는다 |

**계좌 콤보는 `***-***-***-01`로 마스킹돼 있고 목록 글자는 그려진 것이라 읽을 수
없다.** 그래서 순번으로 고르고, 순번↔계좌는 `stock-importer/config/account-order.json`에
적어 둔다. 2026-09-06 드롭다운 순서는 신연금저축·종합위탁·CMA(-02)·ISA(-03)·
신연금저축·ISA(-01)·퇴직연금운용이고, `-02`와 `-03`은 우리 대상이 아니다.

##### 화면을 몰면서 조용히 틀렸던 세 가지

다 교차 확인으로 잡았다. 로그만 봤으면 셋 다 "성공"이었다.

1. **계좌가 안 바뀌는데 조회는 됐다.** 다섯 계좌를 돌았지만 화면이 한 번도 바뀌지
   않아 같은 자료를 다섯 번 내놓았다. 계좌마다 마지막 예수금을 견줘서 잡았다.
   콤보는 **칸을 누르고 `Down`**을 쳐야 열린다 - 화살표를 누르거나 `Home`을 먼저
   보내면 열리지 않는다. 번호를 타이핑하는 방법은 버렸다.
2. **날짜 칸은 연·월·일이 나뉜 입력이다.** 가운데를 누르면 월부터 들어가
   `2026-29-06`이 되고, 유니코드 입력은 통하지 않아 `20260906`이 `18000101`이
   됐다. **연도 자리**를 눌러 **실제 키코드**로 보낸다. 넣은 뒤 화면에서 되읽어
   확인한다.
3. **같은 표가 회차마다 다르게 나온다** - `3,613`일 때도 `3613.0`일 때도 있다
   (UIA 셀이냐 COM 값이냐). 그대로 견주면 같은 행이 새 행으로 보여 페이징이
   끝나지 않고 중복이 쌓인다. 숫자를 정규화해 견준다.

##### 거래종류

모르는 거래종류를 만나면 **조용히 넘기지 않고 멈춘다.** 손익이 조용히 틀리는 것보다
낫다. 읽기와 분류를 갈라 두어(`hable-trades --from-file`) 종류 하나 때문에 다섯
계좌를 다시 돌지 않는다.

- `대체입금`은 **은행에서 오는 월 납입**이다(2026-09-06 사용자 확인). 외부 입금이라
  수익률의 분모를 움직인다. 계좌 사이 이동으로 잘못 잡으면 수익률이 조용히 틀린다.
- `일괄대체 입고`는 현금 0이지만 **수량×단가가 취득원가**다.
- `매매제한(출고포함)등록`은 **기록하지 않는다.** 주식을 묶어 둘 뿐 소유권이 그대로라
  ([1285]에 남아 있다) 출고로 적으면 나중에 팔 때 원가가 사라진다.

##### 실현손익 (`portfolio/realized.py`)

계좌·종목별 평균단가를 들고 날짜순으로 훑는다. 실현손익 = 매도 정산금액 − 판 만큼의
평균 취득원가 + 배당 + 이자 − 수수료 − 세금. 정산금액은 이미 세후 실입금이라 따로
빼지 않는다.

**원가를 모르는 매도를 반드시 센다.** 1년 창 이전에 산 주식은 취득원가를 알 수 없고,
수량이 비어 있으면 계산할 방법조차 없다. 그대로 두면 매도대금이 통째로 이익이 되면서
아무 표시도 남지 않는다 - 실제로 2026-02-24 매도 1,336만원이 그렇게 잡혔다.
`unknown_cost_sales`로 세어 `/stock/performance/`가 **"이 금액은 실제보다 큽니다"**를
빨간 글씨로 밝힌다.

2026-09-06 첫 수집: 5계좌 59건, 누적 확정손익 +1,386만원(원가 미상 3건 포함).
배당·이자는 원가와 무관하므로 그대로 정확하다.

##### 매일 수집

`hable-import`가 잔고를 넘긴 뒤 **최근 7일 거래내역**까지 읽는다(`--trade-days`).
겹쳐 읽어도 `external_key`가 같은 행을 알아보므로 중복되지 않고, 하루를 걸러도 그
하루가 영영 비지 않는다. 거래내역을 읽지 못해도 잔고 수집은 무르지 않는다.

**주의: 멱등성 때문에 같은 내용을 다시 보내면 서버가 통째로 건너뛴다.** 수량 칸을
뒤늦게 추가했을 때 재전송으로 채워지지 않아 거래 실행 기록을 지우고 다시 보내야
했다. 스키마를 바꾸면 같은 일이 생긴다.

#### 코인처럼 수집기가 닿지 않는 자산 (2026-09-06)

**수량은 사람이 admin에, 가격은 매일 시세에서.** 수량은 자주 바뀌지 않고 가격은 매일
바뀌므로 이렇게 나눈다. `ManualHolding`에 계좌·종목·수량을 넣고, 종목의 `시세 출처`에
`upbit:KRW-BTC`처럼 적는다. `manage.py update_manual_positions`가 그 수량에 오늘
시세를 곱해 `PositionSnapshot`을 만든다 — H-able에서 온 것과 같은 자리라 비중·수익률
화면이 따로 알 필요가 없다.

시세를 못 받으면 **멈춘다.** 어제 가격으로 오늘 평가액을 적으면 조용히 틀린다. 빠진
심볼도 마찬가지로 오류다. 지금은 업비트 공개 API만 안다(인증 없음).

**넣을 자리를 시드에 만들었다(2026-09-08).** 그전까지는 구조만 있고 DB가 비어 있어
수량을 넣으려면 계좌와 종목부터 손으로 만들어야 했다. 이제 시드가 `coin-wallet`
계좌(별칭 `코인`)와 비트코인·이더리움 종목을 `시세 출처`까지 채워 만든다. 실제로
보유한 둘만 둔다 - 안 가진 코인이 목록에 있으면 고를 때 헷갈릴 뿐이다.
사람은 admin의 `직접 입력 보유`에서 **수량만** 넣으면 되고, 목록에 없는 코인은 종목을
하나 추가해 `upbit:KRW-<심볼>`만 적으면 된다.

- `coin-wallet`은 `required: false`다. 시세를 못 받은 날 주식 성과까지 통째로 사라지면
  안 되고, 이 계좌가 생기기 전의 과거 날짜가 전부 불완전으로 바뀌어 성과 이력이
  지워지기 때문이다. 시세가 빠진 날은 리포트의 계좌 상태에 `missing`으로 뜬다.
- `Instrument.clean()`이 `시세 출처`의 형식과 업체를 저장 시점에 검사한다. 오타를
  새벽에 도는 잡이 발견하면 그 코인이 그날 평가액에서 통째로 빠진다.
- admin의 `직접 입력 보유` 목록에 `현재가`·`평가액(지금)` 열이 있다. 수량을 넣고 바로
  확인하라고 둔 것이며 60초 캐시를 쓴다. **저장되는 값은 여전히
  `update_manual_positions`가 그 시각에 직접 받아온 가격이다.**
- 실제 확인: 0.05 BTC × 106,451,000원 = 5,322,550원(2026-09-08, `--dry-run`).

**2026-09-09 코인 1년 외부 일봉 추가.** `InstrumentDailyPrice`에 업비트 공개
`/v1/candles/days`의 KST 일 종가를 저장한다. API 최대 200개 제한을 `to` 커서로
페이지네이션하며, `update_price_history --days 365`를 실제 실행해 BTC 366일 + ETH
366일 = 732행(2025.09.08~2026.09.08)을 넣었다. 과거 매매·수량 이력은 없으므로 현재
BTC·ETH 수량을 전 구간에 고정한 **현재 보유 구성 가격지수**만 계산한다. 실제 과거
평가액이나 투자손익으로 가장하지 않으며 이 한계를 성과 화면에도 표시한다. 평일
`stock_daily_job`은 이 1년 일봉을 먼저 갱신한 뒤 당일 직접입력 잔고를 만든다.

**같은 날 전체 자산분류로 확대.** H-able 잔고의 현재 보유 종목 12개를 공식 종목코드와
대조해 Yahoo Finance 한국시장 심볼로 연결했다. 한국주식 243거래일, 미국주식·기타주식·
금은 각각 244거래일, 코인과 안전자산은 366일을 표시한다(모두 2025.09.08~2026.09.08).
한국·미국·기타·금·코인은 현재 수량 고정 가격지수이고, 안전자산은 현재 현금 금액을
1원 단가로 고정한 0% 지수다. `update_price_history`는 최신 스냅샷의 전체 종목을 대상으로
알려진 심볼을 찾아 갱신한다. 외부 일봉이 없는 새 종목은 기존 실제 관측 계산을 유지한다.

**자산분류 비교 전용 기간 선택 추가.** `/stock/performance/`의 상단 계좌 성과 기간과
자산 포트폴리오 비교 기간을 분리했다. 비교 영역에서 1개월·3개월·6개월·1년을 즉시
고르거나 `class_start`/`class_end` 날짜를 직접 입력한다. 쿼리는 `class_range`로 분리되어
계좌 선택이나 상단 성과 기간을 바꿔도 비교 기간을 보존한다. 직접 입력 종료일은 최신
관측일을 넘지 않으며 역순 날짜는 정규화한다. 320~1440px 무가로 스크롤과 빠른 기간·
직접 입력·범례 동작을 Playwright로 확인했다. 직접 입력 종료일이 잔고 스냅샷보다
과거여도 추정의 기준인 최신 보유 수량을 사용하므로 세 그래프가 비지 않는다.

#### 매일 도는 잡 — `stock_daily_job` (2026-09-06 추가)

**이게 없어서 성과 화면이 첫날에 머물러 있었다.** 기간별 수익률·현금흐름과 낙폭·
자산분류별 수익률은 하루에 한 행씩 쌓이는 `DailyPortfolioMetric`으로 그리는데,
`hable-import`를 도는 스케줄이 아예 없었다. 사람이 손으로 돌린 2026-09-06 하루뿐이라
그래프가 비어 있었다.

평일 18:30에 `collect_stock_op`(H-able 잔고 + 최근 7일 거래내역) → 
`update_manual_positions_op`(코인 시세) 순으로 돈다. 30분 전 `hable_ready_job`이
H-able이 켜져 있는지 미리 확인해
알린다. 둘째 날부터 그래프가 채워지기 시작한다.

#### H-able 과거 성과 — [0354] (2026-09-09 추가)

2026-09-09 모바일·포트폴리오 조회 개선: 금융자산 두 화면의 요약은 반응형 그리드,
모바일의 긴 표는 항목명이 붙은 카드로 전환한다. 성과 그래프는 컨테이너 폭에 맞춘다.
확정손익 UI와 요청 시 계산 호출을 제거했다(원본 거래 기록은 보존).
`?portfolio=<계좌 ID>&range=1m`으로 종합위탁·ISA·개인연금·퇴직연금 중 하나를 선택해
해당 계좌의 현금흐름·손익·낙폭·MDD를 본다. 기간 링크와 조회 폼은 서로의 선택을
보존한다. 선택 계좌에 이력이 없으면 다른 계좌의 성과로 대체하지 않고 빈 상태를 표시한다.
현재 포트폴리오 선택은 계좌 기준이며 한국주식·미국주식 등 자산분류 기준이 아니다.

2026-09-09 자산분류별 중첩 그래프 추가: 선택한 조회 범위와 계좌 안에서
`PositionSnapshot` 평가액과 매수·매도 현금흐름으로 한국주식·미국주식·기타주식·금·
코인·안전자산의 수익률, 관측 고점 대비 현재 낙폭, 관측 구간 MDD를 계산한다.
세 지표는 각각 모든 자산분류 선을 한 SVG에 겹쳐 그리며, 공통 범례 버튼으로 해당
분류의 세 선과 지표 카드를 함께 숨기고 복원한다. 키보드 Space 조작과 모두 숨긴 상태도
지원한다. 현재 분류별 스냅샷은 2026.09.06·09.08 두 번뿐이라 그래프와 카드에 실제
관측 기간·횟수를 표시한다. 스냅샷 1~2회뿐이던 모든 현재 자산분류는 위 외부 일봉의
현재 보유 구성 가격지수로 대체했다. 실제 과거 보유량·입출금·매매손익이 아니라는
한계를 화면에 명시한다.

성과 화면이 수집 시작 이후 이틀만 보여 주던 문제를 해결했다. [0354]
`계좌별수익률추이`가 제공하는 일자·평가금액·입금·출금·투자손익·일일수익률·
시간가중수익률을 실제 기록으로 읽는다. 툴바 인쇄가 만드는
`rpt035400000.dat`를 사용하므로 owner-drawn 표를 OCR하거나 현재 잔고로 과거를
역산하지 않는다.

- `hable-history --send`가 `account-order.json`의 실제 KB계좌 5개를 순회해
  `POST /stock/api/performance-history/`로 보낸다.
- `HableAccountDailyMetric`에 계좌·날짜별로 멱등 저장하고, 다섯 계좌가 모두 있는
  날짜만 이전 평가액 가중 일일수익률로 합친다.
- `/stock/performance/`는 이 이력을 우선 사용하며 코인 지갑 제외 범위를 밝힌다.
- 매일 도는 `hable-import`에도 과거 성과 갱신을 연결했다.
- 실제 반영·공개 확인: 5계좌 × 365일 = 1,825행, 2025.09.09~2026.09.08,
  1년 수익률 +30.25%.
- 2026-09-09 후속 점검: 선택 기간의 손익·입출금·MDD로 요약 타일을 보정했다.
  매번 H-able 기간 버튼으로 종료일을 오늘로 갱신한 뒤 시작일을 1년 전으로 확장한다.
  날짜칸의 `GetWindowText`는 프로그램으로 바뀐 표시값을 반영하지 않는 것을 실측했다.
  따라서 내보낸 표의 최초일·최종일(7일 이내)·연속 날짜·중복을 검증한다.
  재수집보다 오래된 DB 기록은 보존하고, H-able 조회 첫날의 기준점 0이 기존 실제
  일간 수익률을 덮어쓰지 않게 했다. 이번 수집이 연 Report Viewer만 닫는다.

#### 지금 남은 것

- **목표 비중(`PortfolioTarget`)이 0행이다.** 그래서 리밸런싱 제안이 나오지 않고 digest에도
  그 줄이 없다. 원하는 목표 배분은 사람이 정할 값이라 admin에서 입력해야 한다.
- KB 계좌 전체의 성과·MDD는 [0354]의 최근 1년 실제 이력이다. 자산분류별 그래프는
  과거 수량 이력이 없어 한국 상장 종목 Yahoo Finance 일봉·코인 업비트 일봉과 현재
  보유 수량을 고정한 가격지수이며 실제 계좌 투자수익률은 아니다.
...
- `[1285]`와 `[0354]`는 화면번호 입력창으로 자동으로 연다. 로그인 자체는 사람이 해야 한다.
...
- **`/stock/admin/`은 별도 admin이 아니라 뿌리 `/admin/`으로 보내는 리다이렉트다(`/property/admin/`도 같다).** `admin.site.urls`를 두 번 마운트하면 `admin:` URL namespace가 갈라진다. 포트폴리오 모델은 기존 admin에 등록돼 있다.
...
- **금융자산 1년 계좌 성과는 H-able [0354] 범위다.** KB증권 5개 계좌의 실제 이력이다.
  자산분류 그래프는 외부 일봉과 현재 보유 수량을 고정한 가격지수이므로 과거 평가액·
  실제 투자수익률로 해석하면 안 된다.
...
- **Django 테스트에는 DB 생성 권한이 필요하다.** `property_report` 역할에 `CREATEDB`가 없어 `manage.py test`가 테스트 DB를 만들지 못한다. 테스트 동안만 부여했다가 되돌린다(`ALTER ROLE property_report CREATEDB;` → `NOCREATEDB;`).
  권한을 건드리기 어려울 때는 `manage.py test --settings=report_site.test_settings`로 메모리 SQLite에서 돌린다(2026-09-10 240개 통과). 운영과 DB 엔진이 다르므로 PostgreSQL 고유 동작까지 보증하지는 않는다.
