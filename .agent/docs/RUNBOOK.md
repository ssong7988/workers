...
```powershell
cd ..\report-site
Copy-Item .env.example .env    # FINDER_API_TOKEN, POSTGRES_PASSWORD를 채운다. REPORT_PATH_TOKEN은 선택값이다
...
1. 네이버 관심부동산에 단지를 추가하고 디버깅 포트 `9222`의 전용 Edge가 로그인된 상태인지 확인한다.
2. `real-estate-finder/`에서 `python -m real_estate_finder collect-favorites`를 실행한다. 이 명령은 `data/favorites-latest.json`만 갱신하며 서버나 카카오에는 전송하지 않는다.
3. 스냅샷의 단지명이 비어 있지 않은지, 네이버 표기가 무엇인지 확인한다. 아파트가 아닌 유형은 이름을 읽지 못할 수 있으므로 임의로 추정하지 않는다.
4. 새 단지의 지역과 알림 정책을 확인한 뒤 `SearchCondition`을 추가한다. `complex_names`는 부분 일치이므로 `판교`처럼 짧은 별칭 대신 네이버의 전체 단지명을 쓴다.
5. `properties/seed/searches.yaml`과 DB가 일치하는 상태라면 시드를 고치고 `manage.py import_searches`로 반영한다. admin에서 따로 바꾼 운영값이 있다면 import가 덮어쓸 수 있으므로 admin에 직접 추가하고 시드만 같은 값으로 맞춘다.
6. `manage.py check`, `manage.py test`, finder 테스트와 `python -m real_estate_finder check-api`로 활성 조건 수를 확인한다.

새 조건의 첫 실제 스캔에서는 모든 매물이 신규다. `notify_new`가 켜진 조건은 신규 알림을 보내고, 급매 기준이 있는 조건은 기준 이하 매물을 보낼 수 있으므로 카카오 알림을 예상하고 실행한다.
...
# 지금은 REPORT_PATH_TOKEN=을 비워 명시적인 최상위 경로를 사용한다.
# 나중에 토큰을 쓸 때만 생성해 REPORT_PATH_TOKEN=에 넣는다.
...
차트 한 칸이 하루다. 세로선이 그날의 최저~최고이고 그 양 끝에 가로 끝선을 그어 최저·최고 위치를 짚는다. 상자가 1분위~3분위, 붉은 가로선이 평균이다.

**표본이 5건 미만인 날은 1·3분위를 내지 않는다.** 상자를 그리지 않고 최저~최고 선과 평균만 남기며 표에는 `–`를 넣는다. `method="inclusive"`는 1분위를 `(n-1)*0.25` 위치에 놓는데, 이 값이 실제 관측치 위에 정확히 떨어지는 최소 표본이 5건이다. 4건 이하에서는 이웃한 두 값 사이를 보간한 숫자가 나오며, 매물이 2건이면 1분위는 그저 "싼 쪽에서 25% 지점"이라 시세처럼 보이지만 시세가 아니다.
...
- **신규 급매** — 이번 스캔에서 처음 발견된 매물이면서 유효 급매가 이하인 경우. 기존 매물이 나중에 급매가 되거나 더 내려가도 다시 보내지 않는다
