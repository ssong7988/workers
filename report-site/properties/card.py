"""Render matched Django listings as one self-contained card image."""
...
from .notifier import CardItem, format_eok
from .report import _area_text
...
def _row_html(item: CardItem) -> str:
    ...
    detail = " · ".join(
        ...
        for part in (
            ...
            f"전용 {_area_text(listing.exclusive_area_m2)}㎡",
    ...
    return (
        ...
        f'<div class="row-main"><span class="price">'
        f"{html.escape(format_eok(listing.price_won))}</span>{badges}</div>"
...
def _complex_html(name: str, items: list[CardItem]) -> str:
    ...
    return (
        ...
        f'<span class="count">{len(items)}건</span></div>{rows}</section>'
    )
...
def build_card_image(
    ...
    *,
    ...
    card_html = render_card_html(
        items,
        heading=heading,
        generated_at=_now_text(timezone_name),
